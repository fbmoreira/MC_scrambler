...
#define ALIGN512MASK 0x3fULL
#define DOUBLE_IT(a) ((a) <<= 1ULL)
#define HALF(a) ((a) >> 1ULL)
#define DIV16(a) ((a) >> 4ULL)
#define BYTE_INDEX_TO_LONG_INDEX(a) ((a) >> 0x6ULL)
#define LEN_TO_512(a) ((((a) >> 0x6ULL) + 0x8ULL) - (((a) >> 0x6ULL) & 0x7ULL))// & 7 means % 8.. we want to allocate aligned and multiple to/of 64~B, so we make a check of whether we have a multiple of 8 uint64_t. If we don't, add the leftover.
...
//    uint64_t leftover = 0x8ULL - ((chunk_len >> 0x6ULL) & 0x7ULL);  // & 7 means % 8.. we want to allocate aligned and multiple to/of 64~B, so we make a check of whether we have a multiple of 8 uint64_t. If we don't, add the leftover.
  ...
  //  pool->chunklen = (chunk_len >> (0x6ULL)) + leftover;  // alloc multiple of 64~B (8 uint64_t, 1 m512 for avx)
    pool->chunklen = LEN_TO_512(chunk_len);
	pool->base = (uint64_t*)aligned_alloc(64, sizeof(uint64_t) * pool->chunklen * pool->chunks);  // allocate #chunks of #chunk_len
    pool->alloc_tag = (uint64_t***)aligned_alloc(64, sizeof(uint64_t**) * pool->chunks);  // array with addresses of the variables that hold references to the pool. We need to update these guys on reallocations and frees.
...
//                pool->chunks <<= 0x1ULL;
    ...
    if (((uint64_t)pool->base & ALIGN512MASK) != 0)  // check misalignment.
    {
        ...
        memcpy(aux, pool->base, HALF(asize));  // copy the entire previous pool to half the new memory.
        memset(aux + DIV16(asize), 0, HALF(asize));  // zero the next half, since we did not allocate anything yet. We divide asize by 16 because we want half the number of bytes, in portions of 8~bytes (since we are incrementing an uint64_t ptr)
    ...
    for (uint32_t alloci = 0; alloci < HALF(pool->chunks); alloci++)
    {
    ...
    for (uint64_t alloci = HALF(pool->chunks);
